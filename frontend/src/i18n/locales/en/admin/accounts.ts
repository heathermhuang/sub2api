    ...
    accounts: {
      ...
      openai: {
        ...
        bridgeOnboarding: {
          ...
          connectionTargetDesc: 'Keep normal API accounts on the API path. Choose Responses bridge for a trusted upstream that natively implements /v1/responses.',
          apiTarget: 'API',
          apiTargetDesc: 'Official or OpenAI-compatible API with normal account behavior.',
          responsesBridgeTarget: 'Responses bridge',
          responsesBridgeTargetDesc: 'Strict HTTP/SSE forwarding for a native Responses-compatible bridge.',
          bridgeType: 'Bridge setup',
          genericBridge: 'Generic bridge',
          genericBridgeDesc: 'Configure the generic strict Responses fields yourself.',
          chatGPTWebBridge: 'ChatGPT Web bridge (unofficial preset)',
          chatGPTWebBridgeDesc: 'Applies safe starting values for codex-chatgpt-web without storing browser or consumer-session credentials.',
          upstreamProtection: 'Upstream protection',
          bearerProtection: 'Bearer API key',
          bearerProtectionDesc: 'Sub2API authenticates to the bridge with the account API key.',
          privateProtection: 'Private transport',
          privateProtectionDesc: 'No upstream HTTP auth. Use only behind an authenticated tunnel, guarded proxy, or equivalent private route.',
          discoveredTiers: 'Bridge-reported tiers',
          discoveredTiersDesc: 'Select only tiers reported by the signed-in bridge account. Each selected tier is stored as an identity model mapping.',
          discoveredTiersRequired: 'Select at least one tier reported by the ChatGPT Web bridge.',
          forceResponsesSummary: 'Responses is forced for text requests',
          httpOnlySummary: 'Responses WebSocket mode is off',
          billingProbeOffSummary: 'Upstream billing probes are off',
          concurrencyOneSummary: 'Initial account concurrency is 1',
          headerOverrideSummary: 'Header override is open below; add the private route header without exposing its secret elsewhere.',
        },
      ...
      headerOverride: {
        ...
        nameLabel: 'Header {index} name',
        valueLabel: 'Header {index} value',
        removeRow: 'Remove header {index}',
