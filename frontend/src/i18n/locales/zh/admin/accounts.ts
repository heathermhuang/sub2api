    ...
    accounts: {
      ...
      openai: {
        ...
        bridgeOnboarding: {
          ...
          connectionTargetDesc: '普通 API 账号继续使用 API 路径；仅当可信上游原生实现 /v1/responses 时选择 Responses 桥接。',
          apiTarget: 'API',
          apiTargetDesc: '官方或 OpenAI 兼容 API，保留普通账号行为。',
          responsesBridgeTarget: 'Responses 桥接',
          responsesBridgeTargetDesc: '面向原生 Responses 兼容桥接的严格 HTTP/SSE 转发。',
          bridgeType: '桥接设置',
          genericBridge: '通用桥接',
          genericBridgeDesc: '自行配置通用严格 Responses 字段。',
          chatGPTWebBridge: 'ChatGPT Web 桥接（非官方预设）',
          chatGPTWebBridgeDesc: '为 codex-chatgpt-web 应用安全初始值，不保存浏览器或消费者会话凭据。',
          upstreamProtection: '上游保护方式',
          bearerProtection: 'Bearer API Key',
          bearerProtectionDesc: 'Sub2API 使用账号 API Key 向桥接进行认证。',
          privateProtection: '私有传输',
          privateProtectionDesc: '上游 HTTP 不使用认证。仅可用于已认证隧道、受保护代理或等效私有路由。',
          discoveredTiers: '桥接已报告档位',
          discoveredTiersDesc: '仅选择已登录桥接账号实际报告的档位；每个所选档位会保存为同名模型映射。',
          discoveredTiersRequired: '请至少选择一个 ChatGPT Web 桥接已报告的档位。',
          forceResponsesSummary: '文本请求强制使用 Responses',
          httpOnlySummary: 'Responses WebSocket 模式关闭',
          billingProbeOffSummary: '上游计费探测关闭',
          concurrencyOneSummary: '账号初始并发为 1',
          headerOverrideSummary: '下方已展开请求头覆写；请填写私有路由请求头，不要在其他位置暴露密钥。',
        },
      ...
      headerOverride: {
        ...
        nameLabel: '第 {index} 个请求头名称',
        valueLabel: '第 {index} 个请求头值',
        removeRow: '删除第 {index} 个请求头',
