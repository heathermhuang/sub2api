...
// provider-issued Grok credential rotation. Implementations compare the full
// attempted credential document and proxy before publishing the replacement.
...
// oauthRefreshStateUnavailableError lets the background pool distinguish a
// temporary authoritative-state read failure from account credential failure.
...
func (api *OAuthRefreshAPI) RefreshIfNeeded(
	...
	if err != nil {
		if !isOAuthRefreshRequestPath(ctx) {
	...
	if refreshErr != nil {
		...
		return &OAuthRefreshResult{Account: attemptedAccount}, withGrokCredentialFailureSnapshot(refreshErr, attemptedAccount)
	}
