...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			nullInt64(log.CustomDomainID),
			nullString(log.CustomDomain),
			createdAt,
		},
	}
}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	if customDomainID.Valid {
		value := customDomainID.Int64
		log.CustomDomainID = &value
	}
	if customDomain.Valid {
		log.CustomDomain = &customDomain.String
	}
	if imageSize.Valid {
